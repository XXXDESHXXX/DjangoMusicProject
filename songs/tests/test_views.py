...
class SongListAPIViewTest(APITestCase):
    def setUp(self):
        ...
        self.song1 = Song.objects.create(name='Song 1', file='song1.mp3', genre=genre, user=self.user)
        self.song2 = Song.objects.create(name='Song 2', file='song2.mp3', genre=genre, user=self.user)
    ...
    def test_song_list(self):
        ...
        for song_data, expected_song in zip(response.data, [self.song1, self.song2]):
            ...
            self.assertEqual(song_data['file'], f'http://testserver{expected_song.file.url}')
